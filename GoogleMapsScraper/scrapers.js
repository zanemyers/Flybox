...
async function scrapeGoogleShopDetails(browserContext, urls) {
  ...
  const failedGoogleShops = [];

  // sets batch size to 10% of total urls and ensures batch size is at least 1
  ...
  for (let i = 0; i < urls.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map(async (url, batchIndex) => {
        ...
        const page = await browserContext.newPage();
        await addCustomElementSelectors(page);

        try {
          if (index === 0) {
            progressBar(index, urls.length); // Initialize progress bar on first run
          }
          ...
          if (website === Messages.NO_WEB) {
            ...
            extraDetails = await scrapeWebsite(page, website);
          }
          ...
          const shopDetails = {
            ...
            stars,
            reviewCount,
            hasFishingReport: extraDetails.fishingReport,
    ...
    for (const result of results) {
      if (result.success) {
        ...
        progressBar(result.index + 1, urls.length);
      } else {
        failedGoogleShops.push({ url: url, error: err.message });
      }
    }
  }

  // Sequential write to CSV after all batches complete
  for (const shopDetails of allShopDetails) {
    await writeShopDetails(shopDetails);
  }
